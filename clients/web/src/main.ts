...
function boot(): void {
  ...
  const ensureConferenceRuntime = async (): Promise<ConferenceController> => {
    ...
    const task = (async (): Promise<ConferenceController> => {
      ...
      overlay = createConferenceOverlay({ controller, locale: i18n.locale, selfUserId: userId });
      document.body.appendChild(overlay.root);
      if (epoch !== conferenceRuntimeEpoch || session.currentUser()?.id !== userId) {
        controller.destroy();
        overlay.destroy();
  ...
  // GC_MESSENGER_DIRECT_APK_ONLY_START
  // APK update check (T-413) — NATIVE ONLY. The Capacitor bridge sets window.__gcUpdateInfo from
  // App.getInfo versionName + versionCode, never this bundle's APP_VERSION, before it
  // imports this bundle; on web/PWA the global simply never exists and this block is a no-op — the SW
  // banner above owns web updates. One check per boot, fail-safe by construction: a null verdict
  // (offline, 404, junk manifest) shows nothing and nothing retries. A tap hands the manifest url to
  // the system browser (bridge __gcOpenExternal → Capacitor Browser.open), so the OS owns download +
  // installer — no silent traffic, no auto-download (T-413 rule). baseUrl "" is right under the shell:
  // its fetch-rewrite routes bare "/v1/…" strings to the configured server origin.
  try {
    ...
    if (updId) {
      void fetchUpdateStatus(updId.platform, updId.arch, updId.version, { currentBuild: updId.build })
        .then((status) => {
          presentUpdateStatus(status, {
            i18n,
            host: document.body,
        ...
        .catch(() => { /* update advice is never an obstacle */ });
    }
  } catch {
