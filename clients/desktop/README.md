...
Windows release builds produce NSIS EXE, MSI, portable ZIP and a separately signed Tauri updater
archive. The factory supports x64 and ARM64 in GitHub Actions and an explicit native x64 signed mode on
the controlled Windows worker. Unsigned validation remains isolated and can never consume production
credentials. A production build requires Authenticode on the application binary, EXE and MSI, a
trusted timestamp on each, and the independent updater signature.

The one-time certificate purchase, organization verification, secure worker installation, readiness
check and release commands are documented in `docs/WINDOWS_RELEASE.md`.
