...
//
// The native shell lifecycle (clients/web main.ts under Capacitor) asks clients/core
// fetchUpdateStatus() at boot, after reconnect/foreground resume and every 15 minutes, then hands each
// verdict here. Two surfaces, PRODUCT_UX tone («ненавязчиво по умолчанию», §8 — never steal the
// composer, never nag):
//   'update' → a dismissible banner («Доступно обновление X.Y.Z»), visually the PWA update banner's
//              sibling (.gc-pwa-update chrome). «Позже» removes that surface; a later lifecycle check
//              may show it again only while the same update is still actually available.
...
// Mount the surface for a verdict; 'latest' / null (fail-safe silence) mount nothing. The native
// lifecycle owns the returned handle, destroys it before presenting a refreshed verdict, and clears a
// stale banner when a successful later check says this build is current.
