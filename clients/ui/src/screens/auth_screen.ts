...
//   * every sign-up field carries a short hint, the password gets a strength meter, and the
//     consent gate says out loud why the button is still disabled.
...
export function createAuthScreen(deps: AuthScreenDeps): { root: HTMLElement; destroy(): void } {
  ...
  interface FieldOptions {
    hintKey?: string;
    required?: boolean;
    control?: HTMLElement; // wrapper around the password input that reserves the native trailing affordance
  ...
  // ── Password: keep only the platform/browser reveal affordance. Some mobile WebViews draw
  // their own trailing eye for password inputs; adding an app button beside it produced two eyes.
  // The wrapper remains so CSS reserves room for the native control and text never runs under it.
  const buildPasswordControl = (): HTMLElement =>
    el("div", { class: "gc-field-control" }, [passwordInput]);
