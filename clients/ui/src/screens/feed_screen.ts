...
export interface FeedDeps {
  ...
  // Group calls are opened from the group itself. The shell resolves the current active room and
  // either joins it or creates the selected audio/video room.
  onOpenConference?: (chatId: number, video: boolean) => Promise<void> | void;
...
export function createFeedScreen(deps: FeedDeps): { root: HTMLElement; focus(messageId: number): void; destroy(): void } {
  ...
  // Message events deliberately do not carry profile photos: one chat-wide durable payload cannot
  // encode each recipient's avatar-privacy view. The already viewer-filtered member roster is the
  // authority for the small author discs beside incoming group/channel runs.
  let memberAvatarFileIds = new Map<number, number | null>();
  let membersLoaded = false;
  const messageAvatarProfileLoads = new Map<number, Promise<void>>();
  const messageAvatarSignedUrlLoads = new Map<number, Promise<{ url: string; expires_at: number }>>();
  // One timeline can paint several run-tail discs for the same author. Share only the concurrent
  // signed-URL lookup; once it settles the entry is dropped, so bindAvatarImage's error retry still
  // obtains a genuinely fresh signature.
  const messageAvatarApi: Pick<ApiLike, "get" | "resolveUrl"> = {
    get<T>(path: string): Promise<T> {
      const match = /^\/v1\/files\/(\d+)\/url$/.exec(path);
      if (!match) return Promise.reject(new Error(`unsupported message avatar route: ${path}`));
      const fileId = Number(match[1]);
      const existing = messageAvatarSignedUrlLoads.get(fileId);
      if (existing) return existing as Promise<T>;
      // Keep the actual ApiLike call statically recognizable by the API compatibility oracle. Passing
      // the opaque `path` argument through here would hide this shipping route from source inventory.
      const request = api.get<{ url: string; expires_at: number }>(`/v1/files/${fileId}/url`)
        .finally(() => { messageAvatarSignedUrlLoads.delete(fileId); });
      messageAvatarSignedUrlLoads.set(fileId, request);
      return request as Promise<T>;
    },
    resolveUrl(path: string): string { return api.resolveUrl?.(path) ?? path; },
  };
  let messageAvatarBindings: AvatarImageBinding[] = [];
  const bindMessageAvatar = (target: HTMLElement, fileId: number, alt: string): void => {
    messageAvatarBindings.push(bindAvatarImage(target, messageAvatarApi, fileId, alt));
  };
  const clearMessageAvatarBindings = (): void => {
    for (const binding of messageAvatarBindings) binding.destroy();
    messageAvatarBindings = [];
  };
  const normalizedAvatarFileId = (value: unknown): number | null => {
  ...
  let activeConference: { id: string; video: boolean } | null = null;
  let conferenceProbe = 0;
  ...
  const groupCallBtn = deps.onOpenConference
    ? el("button", {
        type: "button",
        class: "gc-icon-btn gc-feed-call gc-feed-group-call",
        title: conferenceText(i18n.locale, "headerStart"),
        "aria-label": conferenceText(i18n.locale, "headerStart"),
        hidden: true,
      }, [icon("phone")])
    : null;

  const openConference = (video: boolean): void => {
    if (!deps.onOpenConference || chatKind !== "group") return;
    void Promise.resolve(deps.onOpenConference(chatId, video))
      .then(() => loadActiveConference())
      .catch((error) => {
        const code = typeof error === "object" && error !== null
          ? String((error as { code?: unknown }).code ?? "")
          : "";
        showStatus(code === "CONFERENCE_FULL"
          ? conferenceText(i18n.locale, "limitReached")
          : describeError(error, i18n));
        void loadActiveConference();
      });
  };

  groupCallBtn?.addEventListener("click", (event) => {
    event.stopPropagation();
    if (activeConference) {
      // Enter with the microphone on and the camera off; video remains one explicit tap away inside
      // a video-capable room, so joining an active call never surprises the user with a camera preview.
    ...
    modals.open("menu", (release) => ({
      close: openMessageMenu({
        ...
        host: modalRoot(root),
        anchor: groupCallBtn,
        label: conferenceText(i18n.locale, "headerStart"),
        items: [
          {
            id: "group-call-audio",
            label: i18n.t("call.startAudio"),
            glyph: "phone",
            run: () => openConference(false),
          },
          {
            id: "group-call-video",
            label: i18n.t("call.startVideo"),
            glyph: "video",
            run: () => openConference(true),
          },
        ],
        onClose: release,
      }).close,
  ...
  const syncCallButtons = (): void => {
    ...
    if (groupCallBtn) {
      const visible = chatKind === "group";
      const label = conferenceText(i18n.locale, activeConference ? "headerJoin" : "headerStart");
      groupCallBtn.setAttribute("title", label);
      groupCallBtn.setAttribute("aria-label", label);
      groupCallBtn.classList.toggle("is-live", activeConference !== null);
      if (visible) groupCallBtn.removeAttribute("hidden");
      else groupCallBtn.setAttribute("hidden", "");
    }
  };

  const loadActiveConference = async (): Promise<void> => {
    const epoch = ++conferenceProbe;
    if (chatKind !== "group") {
    ...
    try {
      const response = await api.get<{
        conference: { id?: unknown; chat_id?: unknown; video?: unknown } | null;
      }>(`/v1/chats/${chatId}/conference`);
      if (disposed || epoch !== conferenceProbe) return;
      const room = response.conference;
      activeConference = room
        && typeof room.id === "string"
        && Number(room.chat_id) === chatId
        ? { id: room.id, video: room.video === true }
        : null;
      syncCallButtons();
    } catch {
      if (disposed || epoch !== conferenceProbe) return;
  ...
  const renderBubble = (msg: Message, group: { head: boolean; tail: boolean }, albumGroup?: Message[]): HTMLElement => {
    ...
    // matching spacer so the bubbles stay on one vertical rail (Telegram's arrangement). The photo id
    // comes from GET /members, whose publicUser() projection already applies this viewer's privacy
    // rules. Initials remain visible until bindAvatarImage has actually loaded the signed media URL.
    if (!v.mine && showAuthors) {
      if (group.tail) {
        ...
        if (sender && "username" in sender) {
          avatar.setAttribute("data-sender-id", String(sender.id));
          if (!memberAvatarFileIds.has(sender.id)) requestMessageSenderAvatar(sender.id);
          fileId = memberAvatarFileIds.get(sender.id) ?? null;
        }
        if (fileId !== null) bindMessageAvatar(avatar, fileId, v.author);
        row.append(avatar);
      } else {
        row.append(el("span", { class: "gc-bubble-avatar is-spacer", "aria-hidden": "true" }));
      }
    }
    row.append(bubble);
  ...
  const paint = (opts: { keepBottom?: boolean; anchorTop?: number } = {}): void => {
    ...
    clearMessageAvatarBindings(); // cancel signed-URL/image work owned by the prior DOM render
  ...
  const unsubEvents = events.subscribe((evt) => {
    ...
    const p = evt.payload as {
      ...
      conference?: { id?: unknown; chat_id?: unknown; video?: unknown };
    };
    switch (evt.type) {
      case "chat.delivered":
      case "chat.read": {
      ...
      case "conference.invited": {
        const room = p.conference;
        if (!room || Number(room.chat_id) !== chatId || typeof room.id !== "string") return;
        activeConference = { id: room.id, video: room.video === true };
        syncCallButtons();
        break;
      }
  ...
  // A group can have thousands of members while the lightweight roster request is intentionally
  // paged. For an author outside that first page, fetch their public profile once; the endpoint applies
  // the same per-viewer avatar privacy rules as /members. This also covers a brand-new live sender.
  function requestMessageSenderAvatar(userId: number): void {
    if (
      disposed ||
      !membersLoaded ||
      memberAvatarFileIds.has(userId) ||
      messageAvatarProfileLoads.has(userId)
    ) return;
    const request = api.get<{ avatar_file_id?: number | null }>(`/v1/users/${userId}`)
      .then((profile) => {
        if (disposed) return;
        const fileId = normalizedAvatarFileId(profile.avatar_file_id);
        memberAvatarFileIds.set(userId, fileId);
        // A null result changes no pixels. A visible photo can be attached in place to every current
        // run-tail node, avoiding a full timeline repaint (and duplicate downloads for other authors).
        if (fileId !== null && showAuthors) {
          const selector = `.gc-bubble-avatar[data-sender-id="${userId}"]`;
          for (const avatar of listBox.querySelectorAll<HTMLElement>(selector)) {
            if (!avatar.querySelector("img.gc-avatar-photo")) bindMessageAvatar(avatar, fileId, "");
          }
        }
      })
      .catch(() => { /* a later repaint may retry a transient profile failure */ })
      .finally(() => { messageAvatarProfileLoads.delete(userId); });
    messageAvatarProfileLoads.set(userId, request);
  }

  // ---- members (for @mentions and viewer-filtered message avatars) ----
  const loadMembers = async (): Promise<void> => {
    try {
      const rows = await api.get<ChatMember[]>(`/v1/chats/${chatId}/members`);
      if (!disposed) {
        const nextAvatarFileIds = new Map<number, number | null>();
        for (const member of rows) {
          nextAvatarFileIds.set(member.id, normalizedAvatarFileId(member.avatar_file_id));
        }
        members = rows;
        memberAvatarFileIds = nextAvatarFileIds;
        membersLoaded = true;
        renderSubtitle(); // the group/channel header shows this count
        // History normally paints before the roster request completes. Repaint once the privacy-filtered
        // photo ids arrive; authors outside this page are then resolved through their public profiles.
        if (showAuthors) paint();
      }
    } catch {
      // The timeline must not depend on mention-roster availability. Allow the same privacy-filtered
      // per-author fallback used for members beyond the first page.
      if (!disposed) {
        membersLoaded = true;
        if (showAuthors) paint();
      }
  ...
  const loadTitle = async (strict = false): Promise<void> => {
    try {
      ...
      if (entry && !disposed) {
        ...
        renderSubtitle();
        if (chatKind === "group") void loadActiveConference();
        else {
          activeConference = null;
          syncCallButtons();
        }
  ...
  return {
    ...
    destroy() {
      ...
      clearMessageAvatarBindings();
      messageAvatarProfileLoads.clear();
      messageAvatarSignedUrlLoads.clear();
      headerAvatarBinding?.destroy();
