...
// password plate needs for its reveal button, which truncated «Имя пользователя» to «Имя пользова…»
// at font 1.4 while 45 px of the plate stood empty. These tests read the shipped stylesheets and the
// markup contract they depend on.
...
test("a field label reserves the plate gutter, not the reveal button's room", () => {
...
test("the plate that carries a trailing control keeps the wider reserve", () => {
  ...
  assert.match(
    ...
    "the reveal button may end up under the label",
  );
});

test("the markup still emits the control wrapper the reserve keys on", () => {
  const src = readUi("screens/auth_screen.ts");
  // the CSS condition is structural: `.gc-field-control` exists only around an input with a button
  assert.match(src, /class:\s*"gc-field-control"/, "auth_screen.ts stopped wrapping the reveal button");
  assert.match(src, /class:\s*"gc-field-eye"/);
