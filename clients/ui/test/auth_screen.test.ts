...
// gate must say why the button is disabled, and the password plate must not duplicate a native eye.
...
test("password field keeps the native reveal affordance without a duplicate app eye", () => {
  const ui = mount(session);
  const password = ui.input("current-password")!;
  assert.equal(password.attrs["type"], "password");
  assert.ok(ui.byClass("gc-field-control"), "password input still reserves space for the native trailing control");
  assert.equal(ui.byClass("gc-field-eye"), null, "custom reveal button would duplicate the platform eye");

  ui.toggleMode();
  assert.equal(ui.input("new-password")?.attrs["type"], "password");
  assert.equal(ui.byClass("gc-field-eye"), null);
  ui.destroy();
});
