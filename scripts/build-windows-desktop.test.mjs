...
test("contract fails closed when Windows is unsigned or ships no tdjson.dll", () => {
  const unsignedWorkflow = "windows-x64 windows-arm64 TAURI_SIGNING_PRIVATE_KEY";
  ...
  assert.ok(problems.some((value) => value.includes("Authenticode")));
});
...
test("workflow confines unsigned builds to validation and uses exact-master latest-wins", () => {
  const workflow = source(".github/workflows/windows-artifacts.yml");
  const signedSection = workflow.split("  build-signed:")[1]?.split("  assemble-signed-candidate:")[0] ?? "";
  assert.match(workflow, /validate-windows:[\s\S]*--allow-unsigned/);
  assert.doesNotMatch(signedSection, /--allow-unsigned/);
  assert.match(workflow, /Unsigned validation installers are never uploaded or published/);

  assert.match(workflow, /workflow_dispatch:/);
  assert.doesNotMatch(workflow, /^\s+push:\s*$/m);
  assert.match(workflow, /paid Actions minutes are not part of the production release path/);
  assert.match(workflow, /git rev-parse origin\/master/);
  assert.match(workflow, /windows-release-inputs\.mjs/);
  ...
  assert.ok(workflow.includes("GC_VCPKG_ROOT: C:\\vcpkg"));
  assert.doesNotMatch(workflow, /\$env:VCPKG_ROOT/);
  assert.match(workflow, /Reject artifact superseded during build/);
  assert.match(workflow, /cancel-in-progress: false/);
  assert.doesNotMatch(workflow, /clients\/third_party\/tdlib\/\*\*/);
